...
- Adicionalmente: *ELEMENT, TYPE=SPRINGA + *SPRING para fundacao Winkler
  em todos os nos do dura_outer (INCLUINDO perturbation_strip, se existir ->
  parede/fundacao dural UNIFORME ao redor de toda a circunferencia).
...
def write_winkler_inp(
    ...
    """Cria nos fantasmas + elementos SPRINGA para Winkler na parede externa da dura.

    `dura_outer_face_indices` deve conter TODAS as faces da parede externa
    (dura_outer + perturbation_strip, se este existir) para uma fundacao
    UNIFORME. `perturbation_face_indices` continua sendo um conjunto OPCIONAL
    de faces a EXCLUIR (default vazio -> nada excluido -> dura constante).

    Cada no real da parede (que NAO esteja na lista de exclusao) ganha:
    ...
    """
    # Conjunto de nos da parede externa da dura, menos eventual exclusao.
    pert_nodes = set()
    ...
    with open(out_path, "w") as fh:
        fh.write("** Winkler foundation springs cobrindo TODA a parede externa da dura\n")
        fh.write("** (dura_outer + perturbation_strip, se existir) -> fundacao UNIFORME\n")
...
def main():
    ...
    if args.no_winkler or not dura_outer_faces:
        ...
        # Dura CONSTANTE: o Winkler cobre TODA a parede externa, unindo as faces
        # do perturbation_strip ao dura_outer (em vez de exclui-las). Assim a
        # fundacao elastica da gordura orbital fica uniforme ao redor de toda a
        # circunferencia da dura (sem faixa de tracao livre / quebra de simetria).
        winkler_faces = list(dura_outer_faces) + list(perturbation_faces)
        if perturbation_faces:
            print(f"[4/5] Escrevendo Winkler em {args.out_winkler} "
                  f"(dura_outer + perturbation_strip = {len(winkler_faces)} faces, UNIFORME)")
        else:
            print(f"[4/5] Escrevendo Winkler em {args.out_winkler} "
                  f"({len(winkler_faces)} faces de dura_outer)")
        write_winkler_inp(
            args.out_winkler,
            dura_outer_face_indices=winkler_faces,
            perturbation_face_indices=[],
            faces=faces,
            points=points,
