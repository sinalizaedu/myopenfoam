#!/bin/bash
# Roda os 2 solvers em paralelo via preCICE (chamado dentro do container).
set -uo pipefail
cd "$(dirname "$0")"

# on-caso-1: roda os 2 solvers em paralelo via preCICE (chamado dentro do container).
# Assume que os blockMeshDicts ja foram gerados via brunaStuff/gen_on_caso_1_blockmesh.py.

echo "=== Limpando antigos ==="
rm -rf solid/[1-9]* fluid/[1-9]* solid/log.* fluid/log.* \
       precice-Fluid-* precice-Solid-* precice-run \
       2>/dev/null || true
