"""Gerador SIMPLES do blockMeshDict do caso on-mestrado-2.

Versao reduzida: apenas 4 zonas concentricas, cilindricas, sem esclera/globo/LC
e sem contact_artoph. Objetivo: forma minima que converge para depois evoluir.

Geometria (mm; scale 0.001 aplicado pelo blockMesh):
    on   : r=0    -1.50, z=0-30, E=30 kPa
    pia  : r=1.50 -1.55, z=0-30, E=3  MPa
    sas  : r=1.55 -2.35, z=0-30, E=tunable (CSF; comeca em 100 kPa)
    dura : r=2.35 -2.50, z=0-30, E=3  MPa

Discretizacao radial: pia=1, sas=6, dura=2 celulas. Tangencial 8/quadrante.
Axial: 30 celulas (1 mm/celula).

Patches:
    posterior_on   : z=0, central + 4 quadrantes ON
    posterior_pia  : z=0, 4 quadrantes pia
    posterior_sas  : z=0, 4 quadrantes sas
    posterior_dura : z=0, 4 quadrantes dura
    anterior_on    : z=L, central + 4 quadrantes ON
    anterior_pia   : z=L, 4 quadrantes pia
    anterior_sas   : z=L, 4 quadrantes sas
    anterior_dura  : z=L, 4 quadrantes dura
    dura_outer     : i_max de todos os blocos dura (cilindrica externa)

Continuidade nodal: blocos compartilham vertices em z=0 e z=L. Bonded por DOF.

Uso:
    python brunaStuff/gen_on_mestrado_2_simple.py
"""
...
// on-mestrado-2 SIMPLES — apenas nervo + bainha (pia/SAS/dura). Sem esclera,
// globo, LC ou contact_artoph. CSF entra so como pressao em posterior_sas.
//
// Geometria (scale 0.001 -> mm; ver brunaStuff/gen_on_mestrado_2_simple.py):
//   on   : r=0    -1.50, z=0-30   (E=30 kPa)
//   pia  : r=1.50 -1.55, z=0-30   (E=3  MPa)
//   sas  : r=1.55 -2.35, z=0-30   (E=100 kPa, soft mas tratavel)
//   dura : r=2.35 -2.50, z=0-30   (E=3  MPa)
//
// Continuidade nodal: blocos compartilham vertices em z=0 e z=30.
//
// Patches:
//   posterior_on    -> z=0, ON central + 4 ON quadrantes  (livre)
//   posterior_pia   -> z=0, 4 pia quadrantes              (livre)
//   posterior_sas   -> z=0, 4 sas quadrantes              (P_CSF)
//   posterior_dura  -> z=0, 4 dura quadrantes             (livre ou fixed)
//   anterior_on     -> z=L, ON central + 4 ON quadrantes  (fixedDisplacement)
//   anterior_pia    -> z=L, 4 pia quadrantes              (fixedDisplacement)
//   anterior_sas    -> z=L, 4 sas quadrantes              (fixedDisplacement)
//   anterior_dura   -> z=L, 4 dura quadrantes             (fixedDisplacement)
//   dura_outer      -> i_max dura (parede lateral)        (livre)
...
def render_boundary():
    ...
    # dura_outer (i_max de todos os blocos dura)
    faces = []; comments = []
    for side in ('E', 'N', 'W', 'S'):
...
def main():
    ...
    n_total = (
        N_RAD_CTR * N_TANG * N_AXIAL          # central
        + 4 * N_RAD_ON   * N_TANG * N_AXIAL   # ON quadrants
        + 4 * N_RAD_PIA  * N_TANG * N_AXIAL   # pia
        + 4 * N_RAD_SAS  * N_TANG * N_AXIAL   # sas
        + 4 * N_RAD_DURA * N_TANG * N_AXIAL   # dura
    )
